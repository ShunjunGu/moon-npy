...
Pinned toolchain（CI 复现基准）：**MoonBit `0.10.11+6ff76a5f9`**（显示形式 `0.1.20260827 (d0aaa07)` 仅作展示）· **NumPy `2.3.4`** · Python `3.14` · Node.js `24.11.0`（WASM verifier）。
...
Clone or download this repository, then open [examples/wasm-demo/index.html](examples/wasm-demo/index.html)
from your local disk in a browser with WASM GC support. The page includes its WASM module and sample
`.npy` files and accepts local
files by drag and drop. From the repository root, build the current source and run the same verifier
used by CI (Node.js `24.11.0`):
...
To refresh the page's embedded module after a source change, run
...
- ✅ **NPZ 写方向（N7，Unreleased）** — `encode_npz` 产出**未压缩**、可被 `np.savez` 家族互操作的
  ...
  `examples/npz_write` 生成 → `interoperability/verify_npz_output.py` 用 `zipfile.testzip()` +
  `np.load(allow_pickle=False)` 逐成员 `array_equal` 判定，CI 强制（含 UTF-8 key 路径）。
...
多个 `.npy` 读入 → `encode_npz` 打包成未压缩 NPZ → 落盘，NumPy 再 `np.load` 逐成员校验
（与 round-trip demo 同构，往上一级容器；**CI 实际执行的命令**）：

```bash
# 1) MoonBit 读 4 个 fixture -> encode_npz -> 写盘（key = 文件名 stem）
moon run examples/npz_write --target native -- _build/n7_out/demo.npz \
    tests/fixtures/f4_2x3_c_le_v1.npy tests/fixtures/i4_4_c_le_v1.npy \
    tests/fixtures/u1_4_c_na_v1.npy tests/fixtures/c8_4_c_le_v1.npy
#    -> npz_write OK: _build/n7_out/demo.npz | 4 members | 1046 bytes

# 2) NumPy 校验：zipfile CRC-32 + 全 ZIP_STORED + np.load + 逐成员 array_equal
python interoperability/verify_npz_output.py _build/n7_out/demo.npz \
    --reference f4_2x3_c_le_v1=tests/fixtures/f4_2x3_c_le_v1.npy \
    --reference i4_4_c_le_v1=tests/fixtures/i4_4_c_le_v1.npy \
    --reference u1_4_c_na_v1=tests/fixtures/u1_4_c_na_v1.npy \
    --reference c8_4_c_le_v1=tests/fixtures/c8_4_c_le_v1.npy
#    -> [verify-npz] PASS
```

非 ASCII key 同样支持（置 UTF-8 名字标志，CI 另行覆盖）：参数写成 `"数据=in.npy"` 形式的
`<key>=<path>` 即可。
...
(8 cores) · MoonBit `0.10.11+6ff76a5f9`（display `0.1.20260827 (d0aaa07)`）· `moonc v0.10.11+6ff76a5f9` ·
...
pub fn encode_npz(members : Array[NpzMember]) -> Result[Bytes, NpyError]
// 全 ZIP_STORED + 真实 CRC-32 + 固定时间戳，逐字节确定性；非 ASCII key 置 UTF-8 标志（GP bit 11）；
// 拒绝面镜像读侧：NpzUnsafeMemberName / NpzDuplicateMember / NpzZip64Unsupported（N7）
...
```text
  ...
  dtype     descr → (DType, ByteOrder) · itemsize · codec
    ...
    ├──►  npz.decode_npz(Bytes) → NpzArchive     （成员透传 reader.decode；未压缩，v0.3.0 C1）
    └──►  npz.encode_npz(Array[NpzMember]) → Bytes（savez 写方向，N7；成员透传 writer.encode）
...
│   ├── writer/            # encode(NpyArray) -> Bytes（字节级对齐 np.save）
│   ├── npz/               # NPZ ZIP 容器读 + 写（np.savez；未压缩；读 v0.3.0 C1，写 N7 + CRC-32 自实现）
...
├── examples/wasm-demo/   # Browser WASM GC demo + Node verifier
...
moonNum 读方向适配（S6）——v0.3.0 只新增 NPZ 容器读取（C1，未压缩），其后仅追加 NPZ 写方向
（N7，`encode_npz`），其余范围未扩大：
...
### NPZ container layer (v0.3.0 C1)

The same by-construction discipline extends to the ZIP container reader in
`src/npz/`:

- **No inflate implementation — the decompression-bomb attack surface does not
  exist.** `decode_npz` accepts only members whose compression method is
  *stored* (method 0); anything else is refused with `NpzCompressedMember`
  before any payload byte is touched. The codebase contains no decompressor,
  so a zip bomb cannot be inflated by this library — the surface is absent,
  not defended against.
- **Path traversal cannot happen.** Member names are compared against the
  central directory as plain strings and are never used to build filesystem
  paths; any name containing a path separator or an absolute-path prefix is
  refused with `NpzUnsafeMemberName`.
- **Duplicate member names are refused** (`NpzDuplicateMember`), closing the
  classic zip-shadowing ambiguity; zip64 archives are refused
  (`NpzZip64Unsupported`) rather than partially trusted; encrypted members
  fall under `NpzBadStructure`.
...
MoonBit `0.10.11+6ff76a5f9`（display `0.1.20260827 (d0aaa07)`）/ NumPy `2.3.4` /
