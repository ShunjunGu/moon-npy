...
Pinned toolchain（CI 复现基准）：**MoonBit `0.10.11+6ff76a5f9`**（显示形式 `0.1.20260827 (d0aaa07)` 仅作展示）· **NumPy `2.3.4`** · Python `3.14`。
...
- **不做其他格式**：GGUF / SafeTensors / Parquet 不在范围内；**NPZ** 自 v0.3.0 起只做**读方向、
  未压缩**容器（C1），`savez` 写方向、deflate 解压、zip64 仍不实现（见 Security 容器层小节的
  by-construction 理由）。
...
MoonBit `0.10.11+6ff76a5f9`（display `0.1.20260827 (d0aaa07)`）/ NumPy `2.3.4` / Python `3.14`，依次跑：`moon fmt`（无 diff）
